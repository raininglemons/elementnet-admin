...
function ProductMetadataForm() {
  ...
  const submitForm = async () => {
    ...
    const mergedMetafields = newMetafield.key && newMetafield.value ? metafields.concat(newMetafield) : metafields;

    // Get all errors for fields
    for (const metafield of mergedMetafields) {
  ...
  useEffect(() => {
    if (currentVariant) {
      setMetafields(clone(currentVariant.metafields) || []);
    }
  }, [
    currentVariant
  ]);
  ...
  useEffect(() => {
    setDirty((currentVariant && !equal(
        currentVariant.metafields,
      newMetafield.key && newMetafield.value ? metafields.concat(newMetafield) : metafields)
    ));
  }, [
    currentVariant,
    newMetafield.key && newMetafield.value ? metafields.concat(newMetafield) : metafields
  ...
  const metaDiff = JSON.stringify(currentVariant.metafields) !==
    JSON.stringify(newMetafield.key && newMetafield.value ? metafields.concat(newMetafield) : metafields);

  // console.log('metafields', metafields, currentVariant);
    ...
    <Card>
      ...
      <CardContent>
        ...
        <Box textAlign="right">
          <Button
            color="primary"
            disabled={!isDirty || !metaDiff || isSubmitting}
            variant="contained"
            onClick={() => submitForm()}
            type="submit"
          >
